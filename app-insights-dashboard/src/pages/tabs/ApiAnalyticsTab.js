...
export default function ApiAnalyticsTab({ range, startDate, endDate }) {
  ...
  const fetchWatchItem = useCallback(async (opName) => {
    ...
    try {
      ...
      const r = await axios.get(`${API_BASE_URL}/api/performance-panel/detail?${p}`);
  ...
  const handleWatchInput = (val) => {
    ...
    searchDebounceRef.current = setTimeout(async () => {
      try {
        const url = API_ENDPOINTS.apiSearch(val.trim(), range, startDate, endDate);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      axios.get(API_ENDPOINTS.topApis(range, sd, ed)),
      axios.get(API_ENDPOINTS.failures(range, sd, ed)),
      axios.get(API_ENDPOINTS.requestRate(range, sd, ed)),
      axios.get(API_ENDPOINTS.responseCompare(sd, ed)),
      axios.get(API_ENDPOINTS.responsePercentiles(sd, ed)),
      axios.get(API_ENDPOINTS.exceptions(sd, ed)),
      axios.get(API_ENDPOINTS.dependencies(sd, ed)),
      axios.get(`${API_ENDPOINTS.spikeDetector.replace("spike-detector", "failure-codes")}?range=${range}`),
      ...
      if (h.status === "fulfilled") {
        ...
        const mapped = Array.isArray(raw) ? raw.map(r => ({ code: String(r.resultCode || r.code || ""), count: r.count || 0 })).filter(r => r.code && r.count > 0).sort((a, b) => b.count - a.count).slice(0, 10) : [];
