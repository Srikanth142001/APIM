...
export default function FailuresPanel() {
  ...
  const fetchTimeline = useCallback(async () => {
    setLoadingMain(true);
    try {
      const r = await axios.get(`${API_BASE_URL}/api/failures-panel/timeline?${baseParams()}`);
  ...
  const fetchOperations = useCallback(async (win) => {
    ...
    try {
      ...
      const r = await axios.get(`${API_BASE_URL}/api/failures-panel/operations?${p}`);
  ...
  const fetchOverall = useCallback(async (win) => {
    try {
      const p = win ? new URLSearchParams({ windowStart: win.start, windowEnd: win.end }) : baseParams();
      const r = await axios.get(`${API_BASE_URL}/api/failures-panel/overall?${p}`);
      setOverall(r.data);
  ...
  const fetchApiDetail = useCallback(async (op, win) => {
    ...
    try {
      ...
      const r = await axios.get(`${API_BASE_URL}/api/failures-panel/detail?${p}`);
      setApiDetail(r.data);
