...
export default function PerformancePanel() {
  ...
  const fetchTimeline = useCallback(async () => {
    setLoadingMain(true);
    try {
      const r = await axios.get(`${API_BASE_URL}/api/performance-panel/timeline?${baseParams()}`);
  ...
  const fetchOperations = useCallback(async (win) => {
    ...
    try {
      ...
      const r = await axios.get(`${API_BASE_URL}/api/performance-panel/operations?${p}`);
  ...
  const fetchOverall = useCallback(async (win) => {
    try {
      const p = win ? new URLSearchParams({ windowStart: win.start, windowEnd: win.end }) : baseParams();
      const r = await axios.get(`${API_BASE_URL}/api/performance-panel/overall?${p}`);
      setOverall(r.data);
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {dateMode === 'range' && (
              <select value={range} onChange={e => { setRange(e.target.value); setZoomWindow(null); }}
                ...
                {[['10m','10 min'],['30m','30 min'],['1h','1 hour'],['6h','6 hours'],['12h','12 hours'],['24h','24 hours']].map(([v,l]) =>
