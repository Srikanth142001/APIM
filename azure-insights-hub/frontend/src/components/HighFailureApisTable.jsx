...
function ExpandedDetail({ row, range, startDate, endDate, compareStart, compareEnd }) {
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    axios.get(API_ENDPOINTS.highFailureApisDetail(getEnvId(), row.operation_Name, range, startDate, endDate, compareStart, compareEnd))
...
export default function HighFailureApisTable({ range, startDate, endDate }) {
  ...
  useEffect(() => {
    ...
    axios.get(API_ENDPOINTS.highFailureApis(getEnvId(), range, startDate, endDate, effectiveCompare.compareStart, effectiveCompare.compareEnd))
